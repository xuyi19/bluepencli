<template>
  <div class="w-full">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-xl font-semibold text-c-ink">文章库</h1>
    ...
    <div class="rounded-2xl p-5 neu mb-6">
      ...
      <div v-if="topicList.length" class="pt-3 border-t border-c-line">
        <div class="text-xs text-c-muted mb-2">按主题</div>
    ...
    <div v-if="showImport" class="rounded-2xl p-5 neu mb-6">
      ...
      <div v-else-if="importMode === 'file'" class="space-y-4">
        <div class="rounded-xl p-4 neu-inset">
          <div class="text-xs text-c-muted leading-6">
            支持 <span class="text-c-body">.txt / .md</span> 单篇或整个文件夹批量导入，
            也支持本工具导出的 <span class="text-c-body">.json 文章包</span>。
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button v-if="pending.length" @click="pending = []"
            class="px-4 py-2 rounded-xl text-xs font-medium neu-sm text-c-muted">清空</button>
        </div>
        ...
        <div v-else-if="skipped.length" class="text-xs text-c-muted leading-6">
      ...
      <div v-else class="space-y-3">
        <div class="rounded-xl p-4 neu-inset">
          <div class="text-xs text-c-muted leading-6">
            粘贴或选择一个文章包 JSON，格式为
            <code class="text-c-body">{ "articles": [ { "title": "...", "content": "..." } ] }</code>。
            这是本工具「导出」功能的逆操作。
          </div>
        </div>
        <textarea v-model="packText" rows="10" placeholder="粘贴 JSON 内容…"
          class="w-full px-3 py-2 rounded-lg text-xs neu-inset outline-none resize-none
        ...
        <div class="flex gap-2">
          ...
          <button @click="packInput?.click()"
            class="px-4 py-2 rounded-xl text-sm font-medium neu-sm text-c-body">选择文件</button>
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0 flex-1">
            ...
            <div class="flex flex-wrap items-center gap-2 mt-2">
              <span v-if="a.source" class="text-xs px-2 py-0.5 rounded-md neu-inset text-c-muted">
                {{ a.source }}
              </span>
    ...
    <div v-else class="rounded-2xl p-10 neu text-center text-c-muted">
    ...
    <div v-if="reading" class="fixed inset-0 z-[100] bg-black/40 flex items-center justify-center p-6"
      ...
      <div class="rounded-2xl neu max-w-3xl w-full max-h-[85vh] flex flex-col overflow-hidden">
        <div class="px-6 py-4 flex items-start justify-between gap-4 border-b border-c-line">
          <div class="min-w-0">
            <div class="text-sm font-medium text-c-ink">{{ reading.title }}</div>
            <div class="text-xs text-c-muted mt-1">
              {{ reading.source }}
          ...
          <button @click="reading = null"
            class="text-c-muted hover:text-c-body text-lg leading-none shrink-0">×</button>
        </div>
        <div class="px-6 py-5 overflow-y-auto text-sm text-c-body leading-8 whitespace-pre-wrap">
          {{ reading.content }}
        </div>
...
async function onPickPack(e) {
  ...
  e.target.value = ''
  toast.info('已读取，点击「解析并导入」确认')
}

async function confirmPending() {
  const keep = pending.value.filter((p) => p._keep)
  if (!keep.length) return toast.warning('没有勾选任何文章')
  ...
  showImport.value = false
  await load()
  toast.success(`已导入 ${list.length} 篇`)
}

async function importPack() {
  try {
    const list = parseJsonPack(packText.value)
    if (!list.length) return toast.warning('文章包里没有有效文章')
...
function exportMine() {
  ...
  toast.success(`已导出 ${mine.value.length} 篇`)
}
