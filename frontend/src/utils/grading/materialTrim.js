...
//
// ⚠️ v0.17.0 起已接入：PracticeView::applyTrim 在载入题目时调用，
//    裁后材料**同时**用于界面展示与批改提交（展示=提交，老师看的就是考生读的）。
//    取舍（用户 2026-09-22 拍板默认启用）：
//      ・大作文（"参考给定资料，但不拘泥于给定资料"）不裁 —— 本来就要通读全卷；
//      ・正则解析不出 / 引用的资料号找不到 / 材料无分则结构 —— 一律整卷原样，
//        宁可不省，也不能让老师看不到依据（分数变虚比多花钱糟得多）。
//    UI 要求：材料区显示「用了哪几则 / 省了多少」，并提供「查看整卷材料」开关。
//
// 三条硬规矩（接入时也不能破）：
