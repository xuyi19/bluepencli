...
![Version](https://img.shields.io/badge/version-0.23.0-8B9D77?style=flat-square)
...
英文名 **BluePencil**，源自西方出版界一个古老的术语——**blue pencil（蓝铅笔）**。

19 世纪，欧美编辑校对手稿时习惯用**蓝色铅笔**批注、勘阅、删改文稿。早期印刷制版靠拍照感光，而蓝色笔迹不会被底片感光——编辑写下的每一处修改都只留在原稿上，不会被印进出版物。久而久之，`blue-pencil` 从名词演变成动词，意为**审阅、勘校、评判文本**。

不过传统蓝铅笔常带有「删改、删减」的意味。本项目取的是它更本源的那层含义——**审慎勘阅、多方批注评议**，弱化审查删节的负面色彩，结合申论公考场景，定名**蓝笔申论**。

申论没有唯一的标准答案，这一点和数学题截然不同——**红笔只用来标记对错、扣分，蓝笔则代表多名阅卷人共同审阅、写下各自的批注**。蓝笔申论模拟的正是真实公考阅卷的圆桌讨论：各位老师先各自独立评判，出现分歧时互相辩论复核，最终综合给出批改结论。分数不是某一个模型拍出来的，而是一场合议的结果。
