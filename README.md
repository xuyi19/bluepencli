...
（跑的是桌面版时，这个目录在 **exe 同级** —— 连同记账库 `data/bluepencil.db` 一起，复制一份 exe 就等于带着自己的记录走。）
...
│
├── docs/
│   ├── README.md                   # docs 目录导航（每一块是什么、谁生成、能不能删）
│   ├── 题库/
│   │   ├── 汇编/                   # 真题汇编：体系/年份/卷别 目录树，每卷 材料.md+参考答案.md（脚本生成）
│   │   ├── 文章库/                 # 内置文章库的可读 Markdown 镜像（由脚本生成）
│   │   └── 真题数据说明.md          # 真题的来源、分层与字段说明（report_doc.py 生成）
│   ├── practice/                   # 练习记录归档（每篇一对 .md + .json，自动生成；后端硬编码此路径）
│   └── 项目文档/
│       ├── 开发记录.md              # 关键决策与踩坑记录
│       ├── 上线部署分析.md          # 上线可行性、成本测算、部署清单
│       ├── 使用说明书.md            # 面向使用者的完整说明：能干什么、怎么用
│       ├── 项目现状与规划.md        # 完成/未完成、优化方向、现有与未来功能
│       └── 管理员端说明.md          # 作者专用 GUI 的工具清单与排错
├── .tools/                         # 开发验证脚本（不参与构建）
...
│   ├── probe-tauri-data.mjs        # 桌面版记账 + 归档：真落盘、真读回、删完真消失（19 项）
│   ├── probe-tauri-grade.mjs       # 桌面版点一次真批改（假 LLM）：验批完自动落盘 + 自动记账
│   ├── test-md-parity.mjs          # md 渲染的两套实现（Python / Rust）逐字比对
...
# 11b) 桌面版的数据通道：记账 + 记录归档（真落盘、真读回、删完真消失）
node .tools/probe-tauri-data.mjs
node .tools/test-md-parity.mjs       # md 渲染两份实现逐字比对（改渲染逻辑后必跑）
node .tools/probe-tauri-grade.mjs    # 点一次真批改（假 LLM）：批完是否自动落盘 / 自动记账
...
| `probe-tauri-data.mjs` | 桌面版**数据通道**的端到端验证：POST 一条记录 → **去磁盘上确认 `.md` 与 `.json` 真的生成了** → 抽查 md 渲染的 9 处规范 → 列表 / 详情 / 取回原文 / 删除 → 删除后还要确认**文件真的消失**（不是只删了索引）。再验记账：`score_rate` 算得对、时间戳与 Python 侧同格式、`/stats` 字段齐全。收尾把自己造的数据全删掉，不污染用户的 `docs/practice/`。为什么要它：这两个通道**失败时是静默的** —— 归档写不进去界面一切正常（前端会退回 IndexedDB），记账写不进去成本报告的数字永远是 0，用户很久之后才会发现 |
| `probe-tauri-grade.mjs` | 桌面版**点一次真批改**的完整路径：注入假 LLM 配置 → 进练习页 → 填表 → 点「答完了」→ 真实等待 → 结果页断言 → **去磁盘确认多了一份记录** → **确认记账多了一条任务** → 清理。为什么非要有它：前两个探针只验"零件"，而批改这条路上还串着「读配置 → Rust 网关转发 → 解析三师 JSON → 合议 → 自动落盘 → 自动上报」—— **中间任何一环断了，界面都不会报错**（前端对后端不可用是静默降级的）。而且这是**唯一不需要真 Key 就能验的完整路径**：假 LLM 只影响内容质量，不影响链路正确性 |
| `test-md-parity.mjs` | **md 渲染有两套实现**（网站版 Python、桌面版 Rust），这个测试用同一份结构在两边各渲染一次、**逐字符 diff**。样本刻意覆盖所有分支（可信度块 / 逐句批注 / 分项 / 扣分 / 分歧裁定 / 采分点 / 亮点 / 建议，以及字段缺失时的章节跳号）。它第一次跑就抓到真问题：Windows 上 Python 的 `write_text` 会把 `\n` 静默转成 `\r\n`，而 Rust 不会 —— 同一份记录两版写出的文件不一样，且没有任何地方会报错 |
...
**第一次用？看[使用说明书](docs/项目文档/使用说明书.md)**——面向使用者：三种打开方式、批改流程、老师圆桌机制、两种练习模式、报告怎么读、数据与隐私。

**接下来做什么？看[后续开发规划](docs/项目文档/后续开发规划.md)**——三份方案合并后的缺口总表与三阶段规划（带工作量、依赖与价值排序）。

**想知道项目做到哪了？看[项目现状与规划](docs/项目文档/项目现状与规划.md)**——现在能实现的功能、已完成（含实现说明）、还没完成（含原因）、后续优化方向与未来功能，一页看清。

**想看对照内部方案的完成度对账？**
- [方案 v4.0 对照盘点](docs/项目文档/方案v4.0对照盘点.md)——V1–V4 逐条对账、验收标准达标情况、口径偏离及原因
- [方案优化第二版对照盘点](docs/项目文档/方案优化第二版对照盘点.md)——分发体系与导入体验、授权激活、三层题库、9 个 Sprint 进度
- [方案优化第一版对照盘点](docs/项目文档/方案优化第一版对照盘点.md)——六大痛点复诊、13 类验收指标判定、评分内核改造落点与代码证据

见 [`docs/项目文档/上线部署分析.md`](docs/项目文档/上线部署分析.md)，涵盖：
...
| 层 | 内容 | 分发方式 |
|---|---|---|
| 公开 | 框架代码 + **2010–2021 年国考真题（24 套 / 119 题）** + **2008–2021 年河北省考真题（16 套 / 62 题）** + 15 道仿真题 | 本仓库，AGPL-3.0 |
| 私有 | **2022 年起的国考真题（9 套 / 45 题）与河北省考真题（4 套 / 13 题）** | 作者定向分发 `.bpq` 题库包，使用者自行导入 |

合计 **53 套 / 239 题**。**不包含**：私有题库、采分点评分标准、`.bpq` 打包与签名工具
—— 这三样属于核心资产，保留所有权利。想拿完整题库请通过微信联系作者（见各处「获取完整题库」入口）。
