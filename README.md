...
│   │   └── publish-single.mjs          # 发布单文件版到 release/：文件名带版本号，历史版本保留
...
├── release/                        # 本机归档区（不进版本库）：每版产物都留着，供回溯
...
**发布约定：产物名带版本号，`release/` 保留每一个版本的产物（只增不删）。**
...
- 历史版本**不清理**：要回溯"某版当时是什么样"，历史包本身就是证据——重新构建出来的并不是当时那一版（依赖版本、题库数据都可能已经变了）
- `release/版本说明.md` 是**本机维护的分发台账**：逐版本记「有什么功能 / 哪个包能发给别人」，**不进版本库**（里面写明哪些产物含私有卷，只给作者自己看）
